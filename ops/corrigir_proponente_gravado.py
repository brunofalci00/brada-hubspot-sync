...
from financeiro_match_common import mesma_entidade
...
def main():
    ...
    correto = {d["id"]: _proponente(d["properties"]) for d in deals}
    ...
    for sheet_id, aba, col_prop, col_id, primeira in ALVOS:
        ...
        ws.batch_update([{"range": rowcol_to_a1(n, col_prop + 1), "values": [[alvo]]}
                         for n, _atual, alvo, _did in mudancas],
                        value_input_option="USER_ENTERED")
        print(f"  [write] {len(mudancas)} celula(s) corrigida(s). Nenhuma outra coluna tocada.")
